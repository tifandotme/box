...
async function writeConfig(configPath: string, config: Config) {
...
export async function syncLedgerSmoke(options: { configDir: string; sourcePath?: string }) {
  const configPath = path.join(options.configDir, "config.json")
  const config = await readConfig(configPath)
  const production = options.sourcePath
    ? JSON.parse(await readFile(options.sourcePath, "utf8"))
    : JSON.parse(run(["n8n-cli", "workflow", "get", config.productionWorkflowId, "--json"]))
